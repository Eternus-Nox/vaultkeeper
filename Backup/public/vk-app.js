...
const SIGNUP_KDF_PARAMS = Object.freeze({
  memoryCost: 262144, timeCost: 3, parallelism: 1, hashLength: 32,
});
...
async function doChangePassword(currentPw, newPw) {
  ...
  // 2. Derive new master + wrap the EXISTING symKey under it
  const newDerived = await callWorker('deriveLogin', {
    username: state.username, password: newPw, kdfParams: SIGNUP_KDF_PARAMS,
  }, 20000);
  ...
  const newKdf = {
    memory_cost: SIGNUP_KDF_PARAMS.memoryCost,
    time_cost: SIGNUP_KDF_PARAMS.timeCost,
    parallelism: SIGNUP_KDF_PARAMS.parallelism,
  };
